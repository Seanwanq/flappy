...
let runCommand (cmd: string) (args: string) =
    ...
    with ex ->
        Error $"Failed to run command: {ex.Message}"
    ...
    if not (File.Exists "flappy.toml") then
        ...
        | Ok config ->
            ...
            | Ok results ->
                ...
                saveLock "flappy.lock" { Entries = lockEntries }
    ...
    if not (File.Exists "flappy.toml") then
        ...
        | Ok config ->
            if not (Directory.Exists "src") then
                ...
                let sources =
                    Directory.GetFiles("src", "*.cpp", SearchOption.AllDirectories)
                    |> String.concat " "
                ...
                if String.IsNullOrWhiteSpace sources then
                    ...
                    | Ok results ->
                        ...
                        let allDefines =
                            config.Build.Defines
                            @ (config.Dependencies |> List.collect (fun d -> d.Defines))
                        ...
                        let typeFlags =
                            ...
                            | "dll"
                            | "shared"
                            | "dynamic" -> if isMsvc then "/LD" else "-shared -fPIC"
                            | _ -> ""
                        ...
                        let outputName =
                            if config.Build.Type.ToLower() = "dll" || config.Build.Type.ToLower() = "shared" then
                                if isMsvc then
                                    config.Build.Output + ".dll"
                                elif RuntimeInformation.IsOSPlatform OSPlatform.OSX then
                                    config.Build.Output + ".dylib"
                                else
                                    config.Build.Output + ".so"
                            else
                        ...
                        Console.WriteLine
                            $"Compiling [{config.Package.Name}] ({config.Build.Arch}, {config.Build.Type})..."
