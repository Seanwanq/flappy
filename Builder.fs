...
let exportCMakeConfig (ctx: BuildContext) (distDir: string) =
    let name = ctx.Config.Package.Name
    let buildType = ctx.Config.Build.Type.ToLower()
    let isLibrary = buildType = "lib" || buildType = "static" || buildType = "dll" || buildType = "shared"
    
    if isLibrary then
        let cmakeDir = Path.Combine(distDir, "cmake")
        if not (Directory.Exists cmakeDir) then Directory.CreateDirectory cmakeDir |> ignore
        
        let libExt = if ctx.IsMsvc then (if buildType.Contains("dll") || buildType.Contains("shared") then ".dll" else ".lib") else (if buildType.Contains("dll") || buildType.Contains("shared") then ".so" else ".a")
        let libName = Path.GetFileName(ctx.Config.Build.Output) + libExt
        
        let cmakeContent = 
            $"""# Generated by Flappy
add_library({name} {(if buildType.Contains("dll") || buildType.Contains("shared") then "SHARED" else "STATIC")} IMPORTED)

set_target_properties({name} PROPERTIES
    INTERFACE_INCLUDE_DIRECTORIES "${{CMAKE_CURRENT_LIST_DIR}}/../include"
    IMPORTED_LOCATION "${{CMAKE_CURRENT_LIST_DIR}}/../lib/{libName}"
)
"""
...
let createDist (ctx: BuildContext) =
    let buildType = ctx.Config.Build.Type.ToLower()
    if buildType = "lib" || buildType = "static" || buildType = "dll" || buildType = "shared" then
        let distDir = "dist"
        // Try to clean dist dir
        try if Directory.Exists distDir then Directory.Delete(distDir, true) with _ -> ()
        Directory.CreateDirectory distDir |> ignore
        
        // 1. Copy headers
        let includeDist = Path.Combine(distDir, "include")
        Directory.CreateDirectory(includeDist) |> ignore
        if Directory.Exists "include" then
            let files = Directory.GetFiles("include", "*.*", SearchOption.AllDirectories)
            for f in files do
                let rel = Path.GetRelativePath("include", f)
                let dest = Path.Combine(includeDist, rel)
                let dir = Path.GetDirectoryName(dest)
                if not (Directory.Exists dir) then Directory.CreateDirectory dir |> ignore
                File.Copy(f, dest, true)
        
        // 2. Copy binaries
        let libDist = Path.Combine(distDir, "lib")
        Directory.CreateDirectory(libDist) |> ignore
        let outputName = 
            ...
            else (if ctx.IsMsvc then ctx.Config.Build.Output + ".lib" else ctx.Config.Build.Output + ".a")
        
        if File.Exists outputName then
            File.Copy(outputName, Path.Combine(libDist, Path.GetFileName(outputName)), true)
            
        // 3. Export CMake Config
